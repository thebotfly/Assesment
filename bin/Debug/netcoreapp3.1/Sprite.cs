   ...
   public class Sprite
    {
        ...
        public void Draw(Matrix3 transform)
        {
            ...
            pos -= (forward / forward.Length()) * Width / 1;
            pos -= (up / up.Length()) * Height / 1;

            //Find the transform rotation in radians 
            float rotation = (float)Math.Atan2(transform.m21, transform.m11);

            //Draw the sprite
            Raylib.DrawTextureEx(Raylib.LoadTexture("Images/tankGreen.png"), pos * 32,
                (float)(rotation * 180.0f / Math.PI), 32, Color.WHITE);

            Raylib.DrawTextureEx(Raylib.LoadTexture("Images/tankBlack.png"), pos * 40,
                (float)(rotation * 180.0f / Math.PI), 32, Color.WHITE);
        }
    }
}
